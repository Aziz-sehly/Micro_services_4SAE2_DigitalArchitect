...
public class UserService : IUserService
{
    ...
    public async Task<UserResponseDto?> CreateUserAsync(UserCreateDto dto)
    {
        ...
        var keycloakId = await _keycloakService.CreateUserAsync(
            ...
            dto.LastName,
            dto.Password,
            dto.Role.ToString().ToLowerInvariant()
        );
    ...
    public async Task<UserResponseDto?> UpdateUserAsync(long id, UserUpdateDto dto)
    {
        ...
        // ← ADD THESE 4 LINES: Sync verified status to Keycloak
        if (dto.IsVerified.HasValue && user.KeycloakId != null)
        {
            await _keycloakService.UpdateEmailVerifiedAsync(user.KeycloakId, dto.IsVerified.Value);
